...
import { ref, onValue, set, remove, push, off, child, get } from 'firebase/database';
...
function VideoCall({ roomId, onLeaveRoom }) {
    ...
    useEffect(() => {
        ...
        onValue(participantsRef, snapshot => {
            const participantsList = snapshot.val() ? Object.keys(snapshot.val()) : [];
    ...
    function handleParticipantsChanged(participantsList) {
        ...
        participantsList.forEach(participantId => {
            if (!peerConnections.current[participantId] && participantId !== currentUser.uid) {
                ...
                if (participantId > currentUser.uid) {
                    pc.createOffer().then(offer => {
                        ...
                        const offerRef = ref(database, `rooms/${roomId}/offers/${participantId}`);
                        set(offerRef, {
                            sdp: offer.sdp,
                            type: offer.type,
    ...
    function collectIceCandidates(roomId, participantId, pc) {
        const iceCandidatesRef = ref(database, `rooms/${roomId}/candidates/${currentUser.uid}/${participantId}`);
        ...
        const remoteIceCandidateRef = ref(database, `rooms/${roomId}/candidates/${participantId}/${currentUser.uid}`);
    ...
    useEffect(() => {
                ...
                snapshot.forEach(childSnapshot => {
                    ...
                    if (data.type === 'offer' && pc.signalingState !== 'stable') {
                        pc.setRemoteDescription(new RTCSessionDescription(data))
                            ...
                            .then(() => {
                                const answerRef = ref(database, `rooms/${roomId}/answers/${data.from}`);
                                set(answerRef, {
        ...
        const answersRef = ref(database, `rooms/${roomId}/answers/${currentUser.uid}`);
    ...
    const toggleMute = () => {
      ...
      const leaveRoom = async () => {
        ...
        if (currentUser) {
          const participantRef = ref(database, `rooms/${roomId}/participants/${currentUser.uid}`);
